...
//       newRow.innerHTML = `<td>${data[i].id}</td><td>${data[i].name}</td><td>${data[i].role}</td><td>${data[i].salary}</td><td>${data[i].department}</td>`;
//       tbody.appendChild(newRow);
//     }
//   };
//   xmlhttp.open("GET", "http://localhost:8888/employees"); //three parameter(req para, url, true(asynchronous)/false(not work in asynchronous))
//   xmlhttp.send();

//   // window.alert("*** (..)Hi this is Sagar(..) ***");
// }
